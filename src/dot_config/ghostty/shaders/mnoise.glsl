#define SCROLL_SPEED 0.008
#define NOISE_SPEED  0.03
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec2 puv = vec2(uv.x, uv.y * ratio) * NOISE_SCALE - vec2(0.0, iTime * SCROLL_SPEED);
  ...
  vec3 fcol = mix(
    mix(mix(col1, col3, smoothstep(0.0, 0.3, noise)), col2, smoothstep(0.0, 0.5, noise)),
    col4, smoothstep(0.0, 1.0, noise));

  fragColor = vec4(ghosttyCol.rgb + fcol * ghosttyCol.a, ghosttyCol.a);
}
